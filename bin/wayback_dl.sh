...
TIME="$TIME" perl -F'\t' -ane 'print if $F[0] le $ENV{TIME}' > wb_temp/obj_urls.txt

cat wb_temp/obj_urls.txt | \
parallel --colsep="\t" --no-notice -j4 'eval wget -a wb_temp/log/{%}.wget.log --warc-file=wb_temp/o/{#} "https://web.archive.org/web/{1}id_/{2}" -O /dev/null'
...
fetch_cb() {
    ...
    jq -c -r '.[] | (.[0]+"\t"+.[1]+"\t"+.[2])' | \
    TIME="$TIME" perl -F'\t' -ane 'print if $F[0] le $ENV{TIME}' | \
